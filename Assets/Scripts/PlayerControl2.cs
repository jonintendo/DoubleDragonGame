...
float inAirControlAcceleration = 3.0f;
	...
	//Punch//
	bool canPunch=true;	
	private float punchRepeatTime = 0.05f;
    private float punchTimeout = 0.15f;

	
	// Are we jumping? (Initiated with jump button and not grounded yet)
	private bool punching = false;
		// Last time the jump button was clicked down
	private float lastPunchButtonTime = -10.0f;
	
	// Last time we performed a jump
	private float lastPunchTime = -1.0f;
	
	// the height we jumped from (Used to determine for how long to apply extra jump power after jumping.)
	private float lastPunchStartHeight = 0.0f;
	...
	void ApplyJumping ()
{
	...
	void ApplyPunching ()
{
	// Prevent punching too fast after each other
	if (lastPunchTime + punchRepeatTime > Time.time)
		return;

	if (IsGrounded()) {
	
		if (canPunch && Time.time < lastPunchButtonTime + punchTimeout) 
			{
	...
	void UpdateSmoothedMovementDirection ()
{
	...
	var v = vv;//0.0f;//Input.GetAxisRaw("Vertical");
	var h = 0.0f;//Input.GetAxisRaw("Horizontal");
	...
	void DidPunch()
{
	...
	//lastJumpStartHeight = transform.position.y;
	lastPunchButtonTime = -10;
	
	_characterState = CharacterState.Punching;
}
...
UpdateSmoothedMovementDirection();
	...
	if(_animation) {
		...
		{	
				animation[punchAnimation.name].speed = punchAnimationSpeed;
